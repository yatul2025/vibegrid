...
/**
 * Sends a 6-digit email change / verification code via email.
 * @param {string} toEmail - Recipient email address
 * @param {string} otp - 6-digit OTP string
 * @returns {Promise<{ sent: boolean, simulated?: boolean, error?: string, messageId?: string }>}
 */
const sendEmailChangeOtpEmail = async (toEmail, otp) => {
  if (!transporter || !config.email.user || !config.email.pass) {
    console.log('\n' + '='.repeat(60));
    console.log(' [EMAIL CHANGE OTP SIMULATION — No Gmail Credentials Set]');
    console.log(` To: ${toEmail}`);
  ...
  try {
    const info = await transporter.sendMail({
      from: `"VibeGrid Security" <${config.email.user}>`,
      to: toEmail,
      subject: `${otp} is your VibeGrid email verification code`,
      text: `Your VibeGrid email verification code is ${otp}. It expires in 10 minutes. Do not share this code with anyone.`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body style="margin: 0; padding: 24px; background-color: #090d16; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif;">
          <div style="max-width: 480px; margin: 0 auto; background: #0f172a; border-radius: 16px; border: 1px solid #1e293b; padding: 32px 24px; color: #f8fafc;">
            <div style="text-align: center; margin-bottom: 24px;">
              <div style="display: inline-block; width: 44px; height: 44px; line-height: 44px; background: linear-gradient(135deg, #3b82f6, #6366f1); color: #ffffff; font-weight: 800; font-size: 22px; border-radius: 12px;">V</div>
              <h1 style="font-size: 20px; font-weight: 700; color: #ffffff; margin: 14px 0 4px 0;">VibeGrid Security</h1>
              <p style="font-size: 13px; color: #94a3b8; margin: 0;">Email Verification Request</p>
            </div>

            <div style="background: #1e293b; border-radius: 12px; padding: 24px 20px; text-align: center; margin-bottom: 24px; border: 1px solid rgba(255, 255, 255, 0.05);">
              <p style="font-size: 14px; color: #cbd5e1; margin: 0 0 16px 0;">Enter this 6-digit code in your Settings to verify this email address:</p>
              <div style="display: inline-block; font-size: 36px; font-weight: 800; letter-spacing: 8px; color: #60a5fa; background: rgba(59, 130, 246, 0.1); padding: 12px 24px; border-radius: 10px; font-family: 'Courier New', Courier, monospace;">
                ${otp}
              </div>
              <p style="font-size: 12px; color: #94a3b8; margin: 16px 0 0 0;">⏱️ This code expires in <strong>10 minutes</strong>.</p>
            </div>
            ...
            <p style="font-size: 12px; color: #64748b; line-height: 1.6; text-align: center; margin: 0;">
              If you didn't request to update your email address on VibeGrid, please secure your account immediately.
            </p>
          </div>
        </body>
        </html>
    ...
    console.log(`[EMAIL CHANGE OTP] Successfully delivered to ${toEmail}. MessageId: ${info.messageId}`);
    return { sent: true, messageId: info.messageId };
  } catch (error) {
    console.error(`[EMAIL CHANGE OTP Error] Failed to deliver to ${toEmail}:`, error.message);
    return { sent: false, error: error.message };
  }
};
